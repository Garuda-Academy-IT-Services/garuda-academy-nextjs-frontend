...
export const localizedMetadata: Record<Locale, Metadata> = Object.fromEntries(
  Object.entries(localeSpecificMetadata).map(([locale, metadata]) => [locale, { ...commonMetadata, ...metadata }])
) as Record<Locale, Metadata>

if (!process.env.HOST_DOMAIN || !process.env.HOST_DOMAIN_TEST) {
  throw new Error('Missing HOST_DOMAIN or HOST_DOMAIN_TEST environment variable')
}
